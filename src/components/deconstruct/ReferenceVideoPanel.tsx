...
import { socialEmbedUrl, youtubeVideoId } from '@/lib/video-url';
...
export function ReferenceVideoPanel({
  ...
  const embedUrl = socialEmbedUrl(videoUrl);
  const hasLocal = Boolean(previewVideoUrl);
  const hasYouTube = Boolean(ytId) && !hasLocal;
  const hasSocial = Boolean(embedUrl) && !hasLocal && !hasYouTube;
  const hasPlayback = hasLocal || hasYouTube;
  ...
  return (
    <section className="flex h-auto min-h-0 flex-col justify-start rounded-xl bg-zinc-900/35 p-2 sm:p-4">
      ...
      <ReelPreviewFrame>
        {hasLocal ? (
          ...
          <SocialVideoEmbed sourceUrl={videoUrl} />
        ) : (
          <div className="absolute inset-0 flex items-center justify-center px-4 text-center text-xs text-zinc-500">
            Preview unavailable
          </div>
        )}

        {hasLocal && showAnnotations ? (
      ...
      {hasPlayback ? (
        <div className="mt-3 shrink-0 space-y-2 sm:mt-5">
          <input
            type="range"
            min={0}
        ...
        <p className="mt-3 text-center text-[11px] text-zinc-500 sm:text-left">
          Use the embedded player above to watch the reference reel.
        </p>
      ) : null}
    </section>
  );
