...
export function AudioBeatGraph() {
  const {
    breakdown,
    currentTime,
    setCurrentTime,
  ...
  const seekAt = useCallback(
    (clientX: number) => {
      ...
      if (nearest.effectId && Math.abs(nearest.time - time) < 0.5) {
        setSelectedEffectId(nearest.effectId);
      }
    },
    [duration, points, setCurrentTime, setSelectedEffectId]
  );

  if (!breakdown) return null;

  return (
    <div className="space-y-2 rounded-2xl border border-zinc-800 bg-[#121215] p-4">
      <div className="flex items-center justify-between font-mono text-xs text-zinc-400">
        ...
        <span className="text-[11px] font-bold text-zinc-300">Keyframe Markers Active</span>
      </div>

      <div
        ref={containerRef}
        className="relative h-[120px] w-full cursor-crosshair overflow-hidden rounded-xl border border-zinc-800 bg-[#1a1a1e]"
        onPointerDown={(e) => {
      ...
      >
        <div className="pointer-events-none absolute inset-0 grid grid-cols-12 grid-rows-4 opacity-15">
          {Array.from({ length: 48 }).map((_, i) => (
            <div key={i} className="border-b border-r border-zinc-300" />
          ))}
        </div>
        ...
        <div
          className="pointer-events-none absolute bottom-0 top-0 z-10 w-0.5 bg-zinc-300"
